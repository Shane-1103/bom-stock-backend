...
app.post("/api/orders", authRequired, requirePermission("orders:create"), async (req,res) => {
  try {
    const { order_id, order_type, tracking_number, customer_id, shop_name, customer_name, status="Pending", deadline_ship_date, warehouse_id, total=0, items=[] } = req.body;
    if (!order_id || !order_type) return res.status(400).json({ error:"ORDER ID and order_type required" });
    const conn = await pool.getConnection();
    try {
      await conn.beginTransaction();
      const [result] = await conn.execute(
        "INSERT INTO orders (order_id,order_type,tracking_number,customer_id,shop_name,customer_name,status,deadline_ship_date,warehouse_id,total,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
        [order_id,order_type,tracking_number||null,customer_id||null,shop_name||null,customer_name||null,status,deadline_ship_date||null,warehouse_id||null,total,req.user.id]
      );
